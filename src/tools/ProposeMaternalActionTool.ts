...
class ProposeMaternalActionTool implements IMcpTool {
  registerTool(server: McpServer, req: Request) {
    server.registerTool(
      ...
      {
        description:
          "Drafts a FHIR Task and (optionally) a FHIR Flag for the patient, representing a recommended maternal-care action that requires clinician sign-off before activating. When MATERNALGUARD_ENABLE_WRITEBACK is true, the resources are posted to the FHIR server in draft status (Task.status=requested, Flag.status=inactive). Otherwise returns a dry-run preview. Edit-restricted to coordination metadata: clinical content (rationale, guideline citations, underlying values) cannot be modified after creation.",
        inputSchema: {
          patientId: z
            .string()
            .nullable()
